...
var Class = function(properties) {
    var klass = function() { return(arguments[0] !== null && this.initialize && $type(this.initialize) == "function") ? this.initialize.apply(this, arguments) : this; };
    $extend(klass, this);
...
var Events = new Class({
    ...
    fireEvent: function(type, args, delay) {
        if (this.$events && this.$events[type]) {
            this.$events[type].each(function(fn) { fn.create({ 'bind': this, 'delay': delay, 'arguments': args })(); }, this);
        }
        return this;
    },
...
Array.extend({
    forEach: function(fn, bind) { for (var i = 0, j = this.length; i < j; i++)fn.call(bind, this[i], i, this); },
...
String.extend({
    test: function(regex, params) { return(($type(regex) == "string") ? new RegExp(regex, params) : regex).test(this); }, toInt: function() { return parseInt(this, 10); }, toFloat: function() { return parseFloat(this); }, camelCase: function() { return this.replace(/-\D/g, function(match) { return match.charAt(1).toUpperCase(); }); }, hyphenate: function() { return this.replace(/\w[A-Z]/g, function(match) { return(match.charAt(0) + "-" + match.charAt(1).toLowerCase()); }); }, capitalize: function() { return this.replace(/\b[a-z]/g, function(match) { return match.toUpperCase(); }); }, trim: function() { return this.replace(/^\s+|\s+$/g, ""); }, clean: function() { return this.replace(/\s{2,}/g, " ").trim(); },
...
Number.extend({
    toInt: function() { return parseInt(this); }, toFloat: function() { return parseFloat(this); }, limit: function(min, max) { return Math.min(max, Math.max(min, this)); },
...
Element.extend({
    ...
    getStyle: function(property) {
        ...
        if (!$chk(result)) {
            ...
            for (var style in Element.Styles) {
                if (property == style) {
                    ...
                    if (property == "border") {
                        var every = result.every(function(bit) { return(bit == result[0]); });
            ...
            if (property.contains("border")) {
                if (Element.Styles.border.contains(property)) {
                    return["Width", "Style", "Color"].map(function(p) { return this.getStyle(property + p); }, this).join(" ");
                } else if (Element.borderShort.contains(property)) {
                    return["Top", "Right", "Bottom", "Left"].map(function(p) { return this.getStyle("border" + p + property.replace("border", "")); }, this).join(" ");
                }
            }
        ...
        if (result && property.test(/color/i) && result.contains("rgb")) {
            return result.split("rgb").splice(1, 4).map(function(color) { return color.rgbToHex(); }).join(" ");
        }
        return result;
    },
...
Event.fix = {
    relatedTarget: function() { if (this.relatedTarget && this.relatedTarget.nodeType == 3)this.relatedTarget = this.relatedTarget.parentNode; },
...
Element.Methods.Events = {
    ...
    removeEvents: function(type) {
        ...
        if (!type) {
            ...
            this.$events[type].keys.each(function(fn) { this.removeEvent(type, fn); }, this);
    ...
    cloneEvents: function(from, type) {
        ...
        if (!type) {
            ...
            from.$events[type].keys.each(function(fn) { this.addEvent(type, fn); }, this);
        }
        return this;
    }
};
...
Elements.extend({
    filterByTag: function(tag) { return new Elements(this.filter(function(el) { return(Element.getTag(el) == tag); })); },
    filterByClass: function(className, nocash) {
        var elements = this.filter(function(el) { return(el.className && el.className.contains(className, " ")); });
        return(nocash) ? elements : new Elements(elements);
    },
    filterById: function(id, nocash) {
        var elements = this.filter(function(el) { return(el.id == id); });
        return(nocash) ? elements : new Elements(elements);
    },
...
Element.Events.domready = {
    add: function(fn) {
        ...
        if (document.readyState && window.webkit) {
            window.timer = function() { if (["loaded", "complete"].contains(document.readyState))domReady(); }.periodical(50);
...
var Fx = {};
Fx.Base = new Class({
    options: { onStart: Class.empty, onComplete: Class.empty, onCancel: Class.empty, transition: function(p) { return-(Math.cos(Math.PI * p) - 1) / 2; }, duration: 500, unit: "px", wait: true, fps: 50 },
...
Fx.CSS.Single = {
    parse: function(value) { return parseFloat(value); }, getNow: function(from, to, fx) { return fx.compute(from, to); },
...
Fx.CSS.Multi = {
    parse: function(value) { return value.push ? value : value.split(" ").map(function(v) { return parseFloat(v); }); },
...
Fx.CSS.Color = {
    parse: function(value) { return value.push ? value : value.hexToRgb(true); },
    getNow: function(from, to, fx) {
        var now = [];
